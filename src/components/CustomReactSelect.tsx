...
const CustomMultiValue = (props: MultiValueProps<OptionType, true>) => {
  ...
  // Safely handle className with proper fallbacks
  const className = `${
    innerProps.className || ""
  } flex items-center just gap-2`.trim();

  // Create updated props with guaranteed string className
  const updatedProps = {
    ...props,
    innerProps: {
      ...innerProps,
  ...
  return (
    ...
    >
      <Avatar className="flex justify-center items-center h-4 w-4 bg-blue-100">
        ...
        <AvatarFallback className="bg-accent text-center text-sm">
          {initials}
        </AvatarFallback>
      </Avatar>
